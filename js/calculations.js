...
const METRIC_TARGETS = {
  equity_ratio:     { E:'10%台を目指す', D:'20%以上を目指す', C:'30%以上を目指す', B:'40%以上で優良', A:'維持・成長投資へ' },
  current_ratio:    { E:'50%台を目指す', D:'100%以上を目指す', C:'150%以上を目指す', B:'200%以上で優良', A:'維持・短期運用も検討' },
  quick_ratio:      { E:'25%台を目指す', D:'50%以上を目指す', C:'75%以上を目指す', B:'100%以上で優良', A:'維持' },
  op_margin:        { E:'黒字転換が最優先', D:'2%以上を目指す', C:'5%以上を目指す', B:'10%以上で高収益', A:'維持・投資余力あり' },
  labor_ratio:      { E:'60%未満に引き下げ', D:'50%未満を目指す', C:'40%未満を目指す', B:'30%未満で優良', A:'適切な人材投資を' },
  loan_month_ratio: { E:'24か月未満に削減', D:'12か月未満を目指す', C:'6か月未満を目指す', B:'3か月未満で優良', A:'維持・次の投資へ' },
};

// 具体的改善アクション（指標別）
const METRIC_ACTIONS = {
  equity_ratio: [
    '毎期の利益を配当に回さず内部留保に積み上げる',
    '役員貸付金・仮払金など純資産を目減りさせる項目を整理する',
    '遊休不動産・投資有価証券など不要資産を売却して純資産を増強',
    '増資またはDES（借入金の株式化）で財務基盤を強化',
  ],
  current_ratio: [
    '売掛金の回収サイクルを短縮する（締め日・支払条件の見直し）',
    '在庫の回転率を上げ、デッドストックを処分して現金化',
    '短期借入を長期借入に切り替えて流動負債を圧縮',
    '当座貸越枠の設定で緊急時の流動性リスクをヘッジ',
  ],
  quick_ratio: [
    '売掛金の早期回収を徹底する（督促プロセスの強化）',
    '受取手形の割引・ファクタリングで即時現金化',
    '在庫過多なら売れ筋への絞り込みと処分セールを実施',
    '手元現金を増やすため不要な定期預金・有価証券を整理',
  ],
  op_margin: [
    '主力商品・サービスの価格を見直す（原価・競合状況を確認の上）',
    '外注費・材料費の仕入先を見直し、コスト削減交渉を行う',
    '不採算案件・商品を整理し、高付加価値案件に集中する',
    '販管費の固定費部分（家賃・リース・サブスク）を棚卸しして削減',
  ],
  labor_ratio: [
    '売上・粗利益を拡大して分母を増やす（単価アップ・新規顧客獲得）',
    'ITツール・自動化を導入して一人当たりの生産性を向上',
    '業務委託・外注の活用で繁閑に応じた人件費の変動費化',
    '採用計画を精査し、補充採用前に業務量・生産性を確認',
  ],
  loan_month_ratio: [
    '毎月の返済額を上回る営業CFの確保を優先目標に設定する',
    '金利の高い借入から低金利への借換えで金利負担を削減',
    '遊休資産の売却で繰上返済の原資を確保する',
    '金融機関と定期的に情報共有し、必要時のリスケ余地を確保',
  ],
};
